...
import LoadingType from "@/shared/components/loading/Loading";
import SelectDate from "./SelectDate";
import ButtonBack from "./ButtonBack";
import SelectTime from "./SelectTime";
import ViewAppointments from "./ViewAppointments";
import ConfirmingAppointment from "./ConfirmingAppointment";
...
function AppointmentChat() {
  const customerData = useSelector(
    (state: RootState) => state.customer.customer_data
  );
  const professionalData = useSelector(
    (state: RootState) => state.customer.professional_data
  );
    ...
    <main className="relative flex flex-col pt-15 h-screen w-full justify-between">
      ...
      <header className="flex flex-row bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 shadow-lg fixed top-0 left-0 w-full py-4 px-10 md:px-15 lg:px-50 z-100000">
        ...
        <div className="flex gap-3 items-center w-full">
          <motion.div
            key={professionalData.profile_avatar_dd}
            ...
            className={`lg:flex justify-center items-center rounded-full select-none relative bg-gradient-to-b ${
              cartoonAvatars[professionalData.profile_avatar_dd].bgGradient
            } py-2 px-2 text-xl`}
          >
            {cartoonAvatars[professionalData.profile_avatar_dd].emoji}
          </motion.div>
          <div>
            ...
            return (
              <div className="w-full flex flex-col gap-10 mt-10">
                ...
                >
                  <div
                    className={`lg:flex justify-center items-center rounded-full select-none relative bg-gradient-to-b ${
                      cartoonAvatars[professionalData.profile_avatar_dd]
                        .bgGradient
                    } py-2 px-2 text-xl`}
                  >
                    {cartoonAvatars[professionalData.profile_avatar_dd].emoji}

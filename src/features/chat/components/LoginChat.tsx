...
import { update_customer_data } from "../slice";
import type { RootState } from "@/app/store";

// api
import { request_to_register_or_login_customer } from "../services";
...
function LoginChat() {
  ...
  const handleCickButton = async () => {
    if (name.length > 0 && isValidEmail) {
      try {
        const fetchAPI = await request_to_register_or_login_customer({
          name,
          email,
        });
        ...
        if (fetchAPI.data.access_token !== undefined) {
          dispatch(

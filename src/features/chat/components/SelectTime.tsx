...
// utils
import { format_hours } from "@/shared/utils/utils";
...
function SelectTime({
  ...
  const allAvailabilities = useSelector(
    (state: RootState) => state.customer.professional_data.availabilities
  );
  ...
  const handleOnClickAvailability = (availability: Availability) => {
    ...
    const text =
      format_hours(availability.start_time) +
      " - " +
      format_hours(availability.end_time);
